...
​            rpm -e --nodeps jdk的rpm软件包的名字，如下图：

 <img src="images/image-20230423170434021.png" alt="image-20230423170434021" style="zoom:50%;" />

1. 将jdk的tar.gz的压缩包上传到/opt目录下
2. 将jdk压缩包解压缩
3. 在/etc/profile.d目录下创建my_env.sh文件（后缀必须是 .sh）
4. 在my_env.sh文件中配置JAVA_HOME环境变量

​                JAVA_HOME=/opt/jdk1.8.0_152 PATH=$PATH:$JAVA_HOME/bin export PATH JAVA_HOME              



1. 执行source /etc/profile.d/my_env.sh使环境变量立即生效，如果不生效就重启XShell
...
在主机上访问http://ip:8080  由于防火墙安全限制问题，无法直接访问tomcat，

解决方案：通过防火墙服务firewalld设置

查看防火墙状态

systemctl status firewalld

​    ![image-20230423170454337](images/image-20230423170454337.png)
...
防火墙开放8080端口，因为我们是虚拟机安装的tomcat,默认防火墙是阻止外部ip访问所有端口的，所以需要开放8080端口

firewall-cmd --zone=public --add-port=8080/tcp --permanent 

firewall-cmd：是Linux提供的操作firewall的一个工具

- --permanent：表示设置为永久有效，关闭系统重启也生效
- --add-port：标识添加的端口
- --zone=public：firewall将所有网络流量分为多个区域（zone）,指定的zone为public
...
我们学习环境下，还可以直接关闭防火墙(注意：生产环境不能关闭防火墙)

[gec@localhost ~]$ systemctl stop firewalld

[gec@localhost ~]$ systemctl disable firewalld

systemctl disable firewalld:表示禁止开机自动启动防火墙服务

systemctl enable firewalld:表示开机自动启动防火墙服务
...
4. 上传MySQL的rpm安装包到/opt/MySQL目录下
...
1. 安装mysql-server时有可能出现以下异常:

2. 1. 错误：依赖检测失败：

libaio.so.1()(64bit) 被 mysql-community-server-5.7.29-1.el7.x86_64 需要

- 1. 通过yum安装缺少的依赖：yum install -y libaio

- 初始化MySQL：mysqld --initialize --user=mysql （目的：生成mysql的临时密码）

- 1. 查看MySQL的临时密码：cat /var/log/mysqld.log

- 启动MySQL服务：systemctl start mysqld

- 连接MySQL：mysql -uroot -p’临时密码’

- 使用临时密码连接MySQL之后修改密码：set password=password('你的密码');
...
查看mysql.user表：select host,user from mysql.user

示例：

 <img src="images/image-20230423170510327.png" alt="image-20230423170510327" style="zoom:67%;" />
...
\2. ::1 是 IPV6 的本地地址，简单理解为 IPV6 版的 127.0.0.1
...
docker加速器（在阿里云官网中搜索容器镜像服务-》镜像工具-》镜像加速器，得到加速地址）
...
docker -v 
 ...
 <img src="images/image-20230423170523862.png" alt="image-20230423170523862" style="zoom: 50%;" />

mysql 创建容器（不挂载目录）

docker run -id -p 33306:3306 --name=mysql57 -e MYSQL_ROOT_PASSWORD=root centos/mysql-57-centos7
