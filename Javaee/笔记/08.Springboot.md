...
文档地址：https://min.io/docs/minio/kubernetes/upstream/index.html?ref=docs-redirect

文件的存储方式：

- 文件存储服务器（自建），比如：minio
- 租用服务器，比如：阿里云 oss，华为云



下载Minio的安装包，解压 ，并且打开cmd，输入以下指令，运行Minio

~~~cmd
minio.exe server d:/file    
~~~
...
<img src="images/image-20230413090537987.png" alt="image-20230413090537987" style="zoom: 50%;" />
...
//	}
	...
	/**
	 * 配置ehcache
	 * ehcache的作用是在shiro作权限控制时将查询到的权限信息放入到缓存中，而不是每次都重新查询
	 * 
	 * @return
	 */
	@Bean
...
整合ehcache的配置，需要新建一个shiro-ehcache.xml
...
![image-20230413091929296](images/image-20230413091929296.png)
...
public class CustomFormAuthenticationFilter extends FormAuthenticationFilter {
    ...
    //认证成功后跳转的页面。在yml里配置的successurl不一定生效，这里配置最好
	@Override
	protected boolean onLoginSuccess(AuthenticationToken token, Subject subject, ServletRequest request,
			ServletResponse response) throws Exception {
		WebUtils.getAndClearSavedRequest(request);
