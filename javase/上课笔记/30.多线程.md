...
![](images/%E5%A4%9A%E7%BA%BF%E7%A8%8B%E5%86%85%E5%AD%98%E5%9B%BE%E8%A7%A3.png)
...
- JMM内存模型的可见性是指，多线程访问主内存的某一个资源时，如果某一个线程在自己的工作内存中修改了该资源，并写回主内存，那么JMM内存模型应该要通知其他线程来从新获取最新的资源，来保证最新资源的可见性。
  ...
  ![image-20220808173800565](多线程.png)

  

## 5.4 举例

* 进入死循环，flag在被修改之后，线程依旧在死循环中，因为运行的时候是使用的flag的副本
...
- 添加volatile，flag被修改后会自动通知正在使用该变量副本的线程，则会跳出死循环
...
输出结果：三个三个结果输出，并且会出现重复卖同一张票、卖出第0张、卖出负数的票
...
![image-20220810105315235](线程池.png)
